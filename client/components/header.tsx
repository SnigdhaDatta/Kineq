...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const syncAuthAndTabState = () => {
      ...
      // Keep session visible if a token exists; backend refresh cookie handles re-issuing access tokens.
  ...
  return (
    <header className="w-full bg-white/60 backdrop-blur-lg border-b-2 border-black sticky top-0 z-50">
      <div
        className={`mx-auto px-2 py-3 sm:px-4 sm:py-4 items-center gap-2 sm:gap-3 ${
          isLoggedIn ? "grid grid-cols-[auto_1fr_auto]" : "flex justify-between"
        }`}
      >
        {/* Logo */}
        ...
        {isLoggedIn && (
          <div
            className="relative min-w-0 justify-self-center"
            ref={dropdownRef}
          >
            <button
